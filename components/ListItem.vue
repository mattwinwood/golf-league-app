<template>
  <ul role="list" class="divide-y divide-white/5">
    <li v-for="(scorecard, i) in props.list"
        ...
        :key="i" :class="[props.active === scorecard.dbId && 'bg-gray-700 font-bold', 'hover:bg-slate-700 bg-opacity-75 cursor-pointer relative flex items-center space-x-4 px-4 py-4 sm:px-6 lg:px-8']">
      <div :id="scorecard.dbId" class="min-w-0 w-full flex-grow flex-auto">
        <div class="flex items-center gap-x-2">
          ...
          <h2 class="min-w-0 text-sm font-semibold leading-6 text-white">
            <a  class="flex">
              <span :class="['truncate text-xl font-semibold', props.active === scorecard.dbId && 'text-green-400']">{{ getPlayerFullName(scorecard.player.name) }} <span class="font-semibold text-sm italic">({{scorecard.player.name}})</span></span>
              <div class="">
